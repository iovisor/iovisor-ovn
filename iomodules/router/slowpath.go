...
func (m *RouterModule) ProcessPacket(p *hover.Packet) (err error) {
	...
	log.Infof("[router-%d]: Packet arrived from dataplane", p.Module_id)
	log.Infof("[router-%d]: pkt_len(%d) port_id(%d) reason(%d)\n", p.Module_id, p.Packet_len, p.Port_id, p.Reason)
	log.Infof("[router-%d]: next_hop: %x out_port:%d out_port_ip:%x\n", p.Module_id, p.Metadata[0], p.Metadata[1], p.Metadata[2])

	log.Debugf("[router-%d]: -----PKT RECEIVED----\n%s\n", p.Module_id, hex.Dump(p.Data[0:p.Packet_len]))

	//How to decode packets?
	packet := gopacket.NewPacket(p.Data[0:p.Packet_len], layers.LayerTypeEthernet, gopacket.Default)
	...
	case SLOWPATH_ARP_LOOKUP_MISS:
		...
		pkt_to_queue.Data = p.Data[0:p.Packet_len]
		pkt_to_queue.Module_id = p.Module_id
		pkt_to_queue.Sense = hover.EGRESS
		port_out := uint16(p.Metadata[1])
		...
		if ethlayer := packet.Layer(layers.LayerTypeEthernet); ethlayer != nil {
			...
			arppkt, arperr := buildArpPacket(eth.SrcMAC, dstmac, int2ip(p.Metadata[2]), int2ip(p.Metadata[0]), layers.ARPRequest)
			if arperr != nil {
				log.Error(arperr)
				return nil
			}
	...
	case SLOWPATH_ARP_REPLY:
		...
		if arplayer := packet.Layer(layers.LayerTypeARP); arplayer != nil {
			...
			log.Infof("[router-%d]: ARP reply '%s is at %s'\n", p.Module_id, net.HardwareAddr(arp.SourceHwAddress).String(), net.IP(arp.SourceProtAddress).To4().String())
